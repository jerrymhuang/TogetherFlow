# Baseline model with a TimeSeriesTransformer summary network

**Variant:** `v0-transformer-online`  
**Addresses:** Summary-network justification

## Why this arm exists

The published summary network is a bidirectional LSTM behind two strided convolutions; nothing in the paper justifies that choice against an alternative. This arm swaps in a TimeSeriesTransformer and changes nothing else — same prior, same observables, same seed, same online training budget — so any difference in recovery or calibration is attributable to the sequence model alone.

## Generative Configuration

| Setting | Value |
|---------|-------|
| Reviewer point addressed | Summary-network justification |
...
| Metric | w | r | v | noise |
|--------|-----|-----|-----|-----|
| NRMSE | 0.657 | 0.098 | 0.250 | 1.021 |
| Log-gamma | -6.308 | -inf | -17.185 | 4.322 |
| ECE | 0.016 | 0.007 | 0.025 | 0.010 |
| Post. Contraction | 0.568 | 0.993 | 0.940 | 0.000 |
...
2. Low contraction for w, noise — the data may not be informative for these parameters; consider a more informative prior or a richer summary network.
