# Baseline complete-pooling model
...
Reproduces the current published model under the seeded harness. Every other variant is read against this arm, so it exists to make those comparisons attributable rather than to answer a review point itself.
...
**Assessment:** Training completed with the following flags: Overfitting detected (avg val/train ratio 9.537x over last 10% of epochs) — reduce capacity, add regularization, or increase simulation budget. Treat the downstream diagnostics as provisional until these are addressed.
...
| Metric | w | r | v | noise |
|--------|-----|-----|-----|-----|
| NRMSE | 0.483 | 0.086 | 0.387 | 0.582 |
| Log-gamma | -inf | -inf | -inf | -inf |
| ECE | 0.392 | 0.261 | 0.350 | 0.380 |
| Post. Contraction | 0.968 | 0.998 | 0.975 | 0.951 |
...
1. Overfitting detected (val/train loss ratio 9.537x in final 10% of epochs) — reduce network capacity, add regularization, or increase simulation budget.
...
4. Overconfident posteriors for w, r, v, noise — inspect the simulator for potential issues and consider increasing the simulation budget.
