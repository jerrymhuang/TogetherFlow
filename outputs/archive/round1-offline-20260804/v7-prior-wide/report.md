# Prior sensitivity — weakly informative

**Variant:** `v7-prior-wide`  
**Addresses:** Reviewer point 7

## Why this arm exists

Widens every marginal toward uniform. Diagnostics that hold here are not artefacts of the reference prior's concentration.
...
**Assessment:** Training completed with the following flags: Overfitting detected (avg val/train ratio 8.791x over last 10% of epochs) — reduce capacity, add regularization, or increase simulation budget. Treat the downstream diagnostics as provisional until these are addressed.
...
| Metric | w | r | v | noise |
|--------|-----|-----|-----|-----|
| NRMSE | 0.333 | 0.055 | 0.289 | 0.675 |
| Log-gamma | -inf | -inf | -inf | -inf |
| ECE | 0.393 | 0.210 | 0.375 | 0.412 |
| Post. Contraction | 0.969 | 0.999 | 0.977 | 0.934 |
...
1. Overfitting detected (val/train loss ratio 8.791x in final 10% of epochs) — reduce network capacity, add regularization, or increase simulation budget.
