# Prior sensitivity — sensing radius bounded to the room

**Variant:** `v7-prior-bounded-radius`  
**Addresses:** Reviewer points 7 and 2

## Why this arm exists

The reference prior lognormal(0, 0.5) puts mass beyond the room diagonal (~6.4 for an 8x10 room), where every agent already senses every other agent and r stops being distinguishable. Restricting r to [0.25, 4.0] tests whether poor recovery of r is a prior-support artefact rather than a structural non-identifiability.

## Generative Configuration

| Setting | Value |
|---------|-------|
| Reviewer point addressed | Reviewer points 7 and 2 |
...
**Assessment:** Training completed with the following flags: Overfitting detected (avg val/train ratio 8.384x over last 10% of epochs) — reduce capacity, add regularization, or increase simulation budget. Treat the downstream diagnostics as provisional until these are addressed.
...
| Metric | w | r | v | noise |
|--------|-----|-----|-----|-----|
| NRMSE | 0.403 | 0.092 | 0.296 | 0.682 |
| Log-gamma | -inf | -inf | -inf | -inf |
| ECE | 0.393 | 0.235 | 0.350 | 0.415 |
| Post. Contraction | 0.969 | 0.996 | 0.980 | 0.942 |
...
1. Overfitting detected (val/train loss ratio 8.384x in final 10% of epochs) — reduce network capacity, add regularization, or increase simulation budget.
