# Reference model with a diffusion inference network

**Variant:** `v0-diffusion`  
**Addresses:** Estimator control for the partial-pooling work

## Why this arm exists

Partial pooling needs BayesFlow's CompositionalWorkflow, which accepts only a DiffusionModel inference network — FlowMatching is a sibling class, not a subclass, so the isinstance check rejects it regardless of how close the two are theoretically. That leaves the paper mixing two estimators, and this arm is what licenses it: identical prior, observables, seed, summary network and subnet width, differing only in the generative process. If it lands close to v0-reference, the partial-pooling result is not confounded by the estimator switch. Note 'matched' means matched subnet: DiffusionModel brings a noise schedule and prediction type with no FlowMatching counterpart.

## Generative Configuration

| Setting | Value |
|---------|-------|
| Reviewer point addressed | Estimator control for the partial-pooling work |
...
**Assessment:** Training completed with the following flags: Overfitting detected (avg val/train ratio 1.19x over last 10% of epochs) — reduce capacity, add regularization, or increase simulation budget. Treat the downstream diagnostics as provisional until these are addressed.
...
**Assessment:** r, v show contraction in the acceptable range; w, noise fall short.

## Numerical Diagnostic Summary

| Metric | w | r | v | noise |
|--------|-----|-----|-----|-----|
| NRMSE | 0.706 | 0.053 | 0.276 | 0.844 |
| Log-gamma | 1.745 | 0.702 | 2.940 | 2.666 |
| ECE | 0.039 | 0.053 | 0.009 | 0.010 |
| Post. Contraction | 0.522 | 0.997 | 0.917 | 0.269 |
...
1. Overfitting detected (val/train loss ratio 1.19x in final 10% of epochs) — reduce network capacity, add regularization, or increase simulation budget.
...
3. Low contraction for w, noise — the data may not be informative for these parameters; consider a more informative prior or a richer summary network.
4. Fair calibration for r — consider more training epochs or a slight capacity increase.
