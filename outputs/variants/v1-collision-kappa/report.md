# Separation gain inferred

**Variant:** `v1-collision-kappa`  
**Addresses:** R1 point 1

## Why this arm exists

The reviewers' primary concern is that Vicsek carries no forces, so agents pass through one another and the trajectories are not usable as navigation. The separation term answers that directly. Behavioural evidence lives in outputs/scenarios/; these arms answer the different question of whether adding the mechanism costs anything in parameter recovery. Promotes kappa from a setting we chose to a quantity the data speaks to. The prior spans the over-dispersed regime deliberately: if the posterior excludes it, the calibrated value is supported by the data rather than by our judgement of the figures. If kappa turns out non-identifiable, that is the honest result and fixing it is justified.
...
| Inferred parameters | w, r, v, noise, kappa |
...
**Assessment:** r show recovery in the acceptable range; w, v, noise, kappa fall short.
...
**Assessment:** w, r, v, noise, kappa show calibration in the acceptable range.
...
**Assessment:** w, r, v show contraction in the acceptable range; noise, kappa fall short.

## Numerical Diagnostic Summary

| Metric | w | r | v | noise | kappa |
|--------|-----|-----|-----|-----|-----|
| NRMSE | 0.262 | 0.056 | 0.153 | 0.612 | 0.672 |
| Log-gamma | 4.385 | 0.002 | 3.328 | 2.931 | 1.310 |
| ECE | 0.019 | 0.056 | 0.019 | 0.013 | 0.020 |
| Post. Contraction | 0.934 | 0.997 | 0.974 | 0.632 | 0.617 |
...
**kappa** — excellent calibration; poor recovery; low contraction

## Suggested Next Steps

1. Poor recovery for w, v, noise, kappa — increase network capacity and training duration; if no improvement, these parameters may be weakly identifiable.
2. Low contraction for noise, kappa — the data may not be informative for these parameters; consider a more informative prior or a richer summary network.
